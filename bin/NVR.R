...
# Author: Sam Nooij
# Date: 18 Jan 2018
# Version: snakemake-compatible

#For debugging:
#print(snakemake@input[[1]], snakemake@output[[1]])

library('seqinr')
library('data.table')

calculate.nvr <- function(input.file) {
    sequences <- read.fasta(file = input.file)
    nvr.df <- data.frame(
...
nvr.df <- calculate.nvr(snakemake@input[[1]])

write.table(x = nvr.df, file = snakemake@output[[1]], sep = ",", row.names = FALSE, col.names = TRUE)
